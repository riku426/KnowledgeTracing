...
class DataReader():
    ...
    def getData(self, file_path):
        ...
        with open(file_path, 'r') as file:
            for length, skill, ques, ans in tqdm.tqdm(itertools.zip_longest(*[file] * 4)):
                ...
                if length > problem_len:
                    ...
                    ques_diff = [diff_set[ques[i]] for i in range(len(ques))]
                    
                    ans = [int(a) for a in ans.strip().strip(',').split(',')]
                    slices = length//self.maxstep + (1 if length % self.maxstep > 0 else 0)
                    for i in range(slices):
                        temp = temp = np.zeros(shape=[self.maxstep, self.numofdiffs * 2 * self.numofques])
                        if length > 0:
                            ...
                            for j in range(steps):
                                if ans[i*self.maxstep + j] == 1:
                                    temp[j][skill[i*self.maxstep + j] + ques_diff[i*self.maxstep + j] * 2 * self.numofques] = 1
                                else:
                                    temp[j][skill[i*self.maxstep + j] + ques_diff[i*self.maxstep + j] * 2 * self.numofques + self.numofques] = 1
